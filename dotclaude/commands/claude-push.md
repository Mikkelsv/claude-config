...
Commit and push all changes in the `~/Documents/Code/claude-config/` config repo.

Scripts directory: `~/.claude/scripts`

## Steps

1. Review what changed using `git -C ~/Documents/Code/claude-config diff` and `git -C ~/Documents/Code/claude-config status --porcelain` to write a good commit message. Keep it concise (one line, imperative mood).
