...
2. Read the diff (`git diff` + `git diff --cached` + `git ls-files --others --exclude-standard`) to pick TYPE and write the message.
...
4. Run `~/claude-config/Claude/scripts/commit.ps1 -Message "TYPE: msg" [-Body "details"] [-Amend] -Push`
