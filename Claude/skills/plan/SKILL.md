...
Once discovery is complete, create a plan file in the project's plans directory (default: `plans/`; check `Claude/local/skills/plan/config.md` if it exists for a custom path). Read `plan-template.md` (in the implement skill directory — check `Claude/skills/implement/plan-template.md` first, then `~/claude-config/Claude/skills/implement/plan-template.md`) for the task format — tasks must follow that structure exactly so `/implement` can parse them.
...
If `Claude/local/skills/plan/config.md` exists, read it for:
- **Plan directory** (default: `plans/`)
- **Feature board path** (optional — e.g., `plans/FEATURES.md`)
