...
**Agent A — Boundaries**: Module responsibilities bleeding? Dependency direction violations? Reverse/circular deps? Abstraction level mismatches? Check `Claude/local/skills/audit-architecture/config.md` for project-specific rules if it exists.
...
If `Claude/local/skills/audit-architecture/config.md` exists, read it for architecture boundary rules.
