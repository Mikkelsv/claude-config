...
- **When work is ready to merge, invoke `/rebase-on-main`.** Handles rebase, conflict resolution, build verify, a `/test` run, then a merge prompt (no-ff merge / fast-forward / squash / build & test / cancel). A non-green test verdict warns in the prompt but never blocks — in a repo without CI that prompt is the last backstop before main. Don't run raw `git merge` / `git push origin main`.
